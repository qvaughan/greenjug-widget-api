...
- Change the plugin version to 3.0.0.
- Update build.sbt with
    - .enablePlugins(PlayJava, PlayEbean)
    - "mysql"                           % "mysql-connector-java"                % "5.1.36",

- Create the widgets schema in MySQL
- Update application.conf with database configurations — Point out you can also make settings inside of application.conf as environment variables or override in separate files via import.
play.evolutions {
...
- Create the evolutions file for creating the Widget table.
...
- Create the model class for Widget.
...
- Add ServerConfigStartup to use binary UUIDs
package models.db;
...
- Create models.dto package and create WidgetRequest.
package models.dto;
...
- Create Widget Service interface, add methods to create a Widget, list Widgets, get a Widget, update a widget.
package services;
...
- Create Widget Service Implementation.
...
- Wire up WidgetService to WidgetServiceImpl in Module.


        bind(WidgetService.class).to(WidgetServiceImpl.class);

- Create Widget controller to create a widget.
package controllers;
...
- Add route to to top of routes for creating a widget.
POST    /widgets                        controllers.WidgetController.create

- Use Postman to create a widget. ( 1. Create Widget )

- Add 2.sql evolution to add color column to the Widget table.
# --- !Ups
...
- Add color property, getter/setter, and update non-id constructor to models.db.Widget.

- Add color property and getter/setter, and update toWidget() in dto.WidgetRequest.

- Create a widget in Postman with color property specified.

- Update Widget Controller to list Widgets.
...
- Update WidgetService and WidgetServiceImpl to allow color on list method.

- Update routes to list widgets, allowing filtering with query string.
GET         /widgets             controllers.WidgetController.list(make: String ?= null, model: String ?= null, color: String ?= null)

- Demonstrate listing widgets with Postman, including filtering by make and model. (2. List Widgets)

- Update Widget Controller to retrieve a specific widget.
public Result get(UUID id) {

-     return ok(Json.toJson(widgetService.get(id)));
- }

- Update routes to update a specific widget.
GET         /widgets/:id            controllers.WidgetController.get(id: java.util.UUID)

- Demonstrate getting a specific widget with Postman.

- Add annotation for action to authenticate a secret bearer token.
...
- Update the create/update widget paths to require the secret bearer token annotation.
@With(MagicWordAuth.class)

- Attempt to Create a widget in Postman with and without the “please” Authorization header.
